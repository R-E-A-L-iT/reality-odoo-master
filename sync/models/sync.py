...
class sync(models.Model):
    ...
    def getSyncValues(self, sheetName, psw, template_id, sheetIndex, syncType):
        ...
        if(syncType == "Companies"):
            ...
            # quit, msg = self.start_sync_pricelist(sheetName, sheet)
            _logger.info(str(self.pool.get('sync_pricelist.sync_pricelist')))
    ...
    def syncCCP(self, sheet):
        ...
        while(True):
            ...
            try:
                ...
                ccp_ids = self.env['ir.model.data'].search(
                    [('name', '=', external_id), ('model', '=', 'stock.production.lot')])
                if(len(ccp_ids) > 0):
                    self.updateCCP(self.env['stock.production.lot'].browse(
                        ccp_ids[-1].res_id), sheet, sheetWidth, i, columns)
                else:
