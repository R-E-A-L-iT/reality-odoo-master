    ...
    def normalize_bools(value, field=None):
        if isinstance(value, str):
            ...
            if normalized_value in ["true", "1"]:
                return True
            elif normalized_value in ["false", "0", "", "#ref!", "none", "null"]:
                return False

        if isinstance(value, bool):
            return value  # Return as-is if it's already a boolean

        if isinstance(value, (int, float)):
            return bool(value)  # Convert numbers properly (0 -> False, others -> True)

        return False  # Default to False if the value is unrecognized
