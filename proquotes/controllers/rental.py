...
class RentalCustomerPortal(cPortal):
    # Handle Update of rental fields in quote
    ...
    @http.route(
        ["/my/orders/<int:order_id>/newAddress"],
        type="json",
        auth="public",
        website=True,
    )
    ...
    @http.route(
        ["/my/orders/<int:order_id>/street"], type="json", auth="public", website=True
    )
    ...
    @http.route(
        ["/my/orders/<int:order_id>/zip"], type="json", auth="public", website=True
    )
    ...
    @http.route(
        ["/my/orders/<int:order_id>/state"], type="json", auth="public", website=True
    )
    ...
    @http.route(
        ["/my/orders/<int:order_id>/country"], type="json", auth="public", website=True
    )
    ...
    def checkDates(self, order):
        # Confirm rental date range is valid
        if order.rental_end == False:
            return
        if order.rental_start == False:
            order.rental_end = False

        start_year, start_month, start_day = str(order.rental_start).split("-")
            ...
            if country_rec:
                ...
                if state:
                    state_rec = request.env["res.country.state"].sudo().search(
                        [("name", "ilike", state), ("country_id", "=", country_rec.id)],
                        limit=1,
                    )
                    if state_rec:
    ...
    @http.route(
        ["/my/orders/<int:order_id>/end_date"], type="json", auth="public", website=True
    )
