...
// ─────────────────────────────────────────────────────────────────────────────
// Product demo / lead request form.
//
// Reusable sibling of rtc_demo_request.js: same field-population, in-person /
// virtual toggle, validation and submit behaviour, but keyed on the generic
// `.o_rtc_lead_form` block (the paste-able section dropped onto any product
// page) and POSTing to /product_demo/submit so the created CRM lead is titled
// by the product the visitor is enquiring about.
//
// Runs for every .o_rtc_lead_form on the page (there's normally one), so the
// same block works no matter which page it's pasted into. The optional product
// dropdown is just a <select name="product">; when the page has no variants,
// swap it for <input type="hidden" name="product" value="…"> — this script
// reads [name="product"] either way, so no JS change is needed per page.
// ─────────────────────────────────────────────────────────────────────────────
...
function wireForm(form) {
    ...
    const status = form.querySelector(".o_rtc_lead_status");
    const btn = form.querySelector(".o_rtc_lead_btn");

    // ── In-person / virtual toggle: show the city field only for in-person ──
    const cityWrap = form.querySelector(".o_rtc_lead_city_wrap");
    ...
    form.addEventListener("submit", (ev) => {
        ...
        jsonrpc("/product_demo/submit", data)
            .then((res) => {
                if (res && res.success) {
...
whenReady(() => {
    document.querySelectorAll(".o_rtc_lead_form").forEach(wireForm);
});
